...
function BlinkAPI.render(delta)
	...
	-- blink uv
	local X = math.clamp(LblinkFrame + delta, 0, BLINK_LENGTH)
	local lblink = -BLINK_LENGTH * math.abs(X / BLINK_LENGTH - math.floor(X / BLINK_LENGTH + 0.5))

	local X = math.clamp(RblinkFrame + delta, 0, BLINK_LENGTH)
	local rblink = -BLINK_LENGTH * math.abs(X / BLINK_LENGTH - math.floor(X / BLINK_LENGTH + 0.5))
